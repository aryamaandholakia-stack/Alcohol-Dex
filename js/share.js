...
async function sharePassport(stats, extra = {}) {
  ...
  ctx.fillText("A L C O H O L D E X", W / 2, 130);
  ctx.fillStyle = "#8A7452";
  ctx.font = `500 22px ${SANS}`;
  ctx.fillText("T H E   P A S S P O R T", W / 2, 188);
  ...
  if (extra.handle) {
    ...
    ctx.fillText(extra.memberSince ? `IN THE ROOM SINCE ${extra.memberSince.toUpperCase()}` : "", W / 2, 338);
  }
  hairline(388);
  ...
  [[String(stats.countries), "COUNTRIES"],
   [`${stats.legendaries}/${stats.legTotal}`, "LEGENDARIES"],
