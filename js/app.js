...
function renderDexHome() {
  ...
  const flagship = DEXES.filter(d => d.group === "flagship");
  const fields = DEXES.filter(d => d.group === "field");
  const cities = DEXES.filter(d => d.group === "city");
  grid.innerHTML =
    ...
    fields.map(d => box(d, "")).join("") +
    `<div class="dex-section-label rise" style="margin-top:1.6rem">City dexes — one city, done properly</div>` +
    cities.map(d => box(d, "")).join("");
...
function renderIntake() {
  ...
  const show = !d.hunt && ids.length && !intakeDone() && dexCollectedCount() === 0;
...
function renderGrid() {
  ...
  $("bottle-grid").innerHTML = bottles.map(b => {
    ...
    return `<div class="card ${found ? "" : "locked"} ${peek ? "peek" : ""} ${b.leg ? "legendary" : ""} ${tcls}" data-id="${b.id}" tabindex="0" role="button" aria-label="${found || peek ? b.name : "A stranger — " + b.cat}">
      ...
      <div class="card-meta"><span class="card-no">№ ${String((INDEX_IN_DEX[b.id] || {no:0}).no).padStart(3, "0")}</span><span class="card-tiermark ${b.tier ? "tm-" + b.tier : ""}">${b.leg ? "◆" : b.tier === "r" ? "●●●" : b.tier === "u" ? "●●○" : b.tier === "c" ? "●○○" : ""}</span></div>
...
function renderProfile() {
  ...
  const taste = (passScope === "all" || passScope === "alcohol") ? tasteProfile(alcoholLogged) : null;
  ...
  $("badges-section").classList.toggle("hidden", passScope !== "all");
...
// ---------------------- DUO PASSPORTS ----------------------
let duoPartner = null;   // { uid, handle }
async function renderDuo() {
  const box = $("duo-section");
  if (!box) return;
  box.classList.toggle("hidden", passScope !== "all");
  if (passScope !== "all") return;
  if (DEMO_MODE || !sb) { $("duo-body").innerHTML = `<p class="lede">Duos need a live account.</p>`; return; }
  const rows = await circle.leaderboard();
  const friends = (rows || []).filter(r => r.uid !== state.user.id);
  if (!friends.length) { $("duo-body").innerHTML = `<p class="lede">Add someone to The Circle first — a duo takes two.</p>`; return; }
  $("duo-body").innerHTML = `<div class="duo-pick">${friends.map(f =>
    `<button class="chip ${duoPartner && duoPartner.uid === f.uid ? "on" : ""}" data-uid="${f.uid}" data-h="${f.handle}">${f.handle}</button>`).join("")}</div>
    <div id="duo-panel"></div>`;
  $("duo-body").querySelectorAll(".chip").forEach(c => c.addEventListener("click", async () => {
    duoPartner = { uid: c.dataset.uid, handle: c.dataset.h };
    renderDuo();
  }));
  if (duoPartner) await renderDuoPanel();
}

async function renderDuoPanel() {
  const panel = $("duo-panel");
  if (!panel || !duoPartner) return;
  panel.innerHTML = `<p class="lede">Consulting the ledger…</p>`;
  const { data, error } = await sb.rpc("alcoholdex_duo_logs", { partner: duoPartner.uid });
  if (error) { panel.innerHTML = `<p class="lede">The room didn't catch that.</p>`; return; }
  const theirs = new Set((data || []).map(r => r.bottle_id).filter(id => byId[id]));
  const mine = state.logged;
  const union = new Set([...mine, ...theirs]);
  const both = [...mine].filter(id => theirs.has(id));
  const rows = DEXES.map(d => {
    const pool = CATALOGS[d.id] || [];
    const u = pool.filter(b => union.has(b.id)).length;
    const bo = pool.filter(b => mine.has(b.id) && theirs.has(b.id)).length;
    return `<div class="cat-row" style="--dexac:${d.accent}"><span class="cat-name"><span class="dex-row-glyph">${d.glyph}</span> ${d.name}</span>
      <span class="cat-count">${u} <span class="of">/ ${pool.length} · ${bo} together</span></span></div>`;
  }).join("");
  panel.innerHTML = `
    <p class="lede" style="margin:1rem 0">You two have <span class="gold">${union.size}</span> between you — <span class="gold">${both.length}</span> lived by both.
    ${theirs.size === 0 ? "If their side reads zero, they haven't added you back yet — duos need a mutual add." : ""}</p>
    ${rows}`;
}
