...
/* Per-dex honors — five per dex, generated from the registry.
   Tests receive per-dex stats {total, legs, catalogTotal, legTotal}. */
function buildDexBadges() {
  const out = [];
  DEXES.forEach(d => {
    const noun = d.noun;
    out.push(
      { id: d.id + "-first", dex: d.id, name: "First " + noun, desc: "Log your first in " + d.name, test: s => s.total >= 1 },
      { id: d.id + "-ten", dex: d.id, name: "Regular of " + d.name, desc: "Ten " + noun + "s in the ledger", test: s => s.total >= 10 },
      { id: d.id + "-half", dex: d.id, name: "Half the shelf", desc: "Half of " + d.name + " lived", test: s => s.total >= Math.ceil(s.catalogTotal / 2) },
      { id: d.id + "-legs", dex: d.id, name: d.legName + ", complete", desc: "Every legendary in " + d.name, test: s => s.legTotal > 0 && s.legs >= s.legTotal },
      { id: d.id + "-all", dex: d.id, name: d.name + ", complete", desc: "The entire collection", test: s => s.total >= s.catalogTotal }
    );
  });
  return out;
}
const DEX_BADGES = (typeof DEXES !== "undefined") ? buildDexBadges() : [];
