...
/* ---------------- CIRCLES — shared group ledgers ---------------- */
const circles = {
  mine: [],
  async list() {
    if (DEMO_MODE || !sb) return [];
    const { data, error } = await sb.rpc("alcoholdex_my_circles");
    this.mine = error ? [] : (data || []);
    return this.mine;
  },
  async create(name) {
    const { data, error } = await sb.rpc("alcoholdex_create_circle", { cname: name });
    if (error) return { ok: false, msg: error.message.includes("CIRCLE_LIMIT") ? "Three circles is the house limit." : "The room didn't catch that." };
    return { ok: true, circle: data[0] };
  },
  async join(code) {
    const { data, error } = await sb.rpc("alcoholdex_join_circle", { ccode: code });
    if (error) return { ok: false, msg: error.message.includes("CIRCLE_LIMIT") ? "Three circles is the house limit." : error.message.includes("CIRCLE_NOT_FOUND") ? "No circle answers to that code." : "The room didn't catch that." };
    return { ok: true, circle: data[0] };
  },
  async leave(id) { if (sb) await sb.rpc("alcoholdex_leave_circle", { cid: id }); },
  async logs(id) {
    const { data, error } = await sb.from("alcoholdex_circle_logs").select("bottle_id, dex_id, logged_at").eq("circle_id", id);
    return error ? [] : (data || []);
  },
  async log(id, bottleId, dexId) {
    return sb.from("alcoholdex_circle_logs").insert({ circle_id: id, bottle_id: bottleId, dex_id: dexId, logged_by: state.user.id });
  },
  async unlog(id, bottleId, dexId) {
    return sb.from("alcoholdex_circle_logs").delete().match({ circle_id: id, bottle_id: bottleId, dex_id: dexId });
  }
};
